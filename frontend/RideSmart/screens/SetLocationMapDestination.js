// RideSmart/screens/SetLocationMapPickup.js
...
import { GOOGLE_MAPS_API as LOCAL_GOOGLE_MAPS_API } from '@env';

const sanitizedLocalKey = LOCAL_GOOGLE_MAPS_API.trim().replace(/^['"]|['";\s]*$/g, '');
const GOOGLE_MAPS_API_KEY = Constants.manifest?.extra?.googleMapsApiKey || sanitizedLocalKey;
...
const fetchPlaceSuggestions = async (input) => {
  ...
  const endpoint = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
    input
  )}&key=${GOOGLE_MAPS_API_KEY}&language=en&components=country:sa&location=${LOCATION}&radius=${RADIUS}`;
...
const fetchPlaceCoordinatesFromPlaceId = async (placeId) => {
  const endpoint = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&key=${GOOGLE_MAPS_API_KEY}`;
  try {
    const response = await fetch(endpoint);
    const data = await response.json();
